/* ======================================================================
   IMEJE-BKZS — Simulation Engine (app.js)
   Enhanced: logo animation, pre-sim dashboard, mini-log feed,
   feature distributions, localStorage sync, keyboard shortcuts
   ====================================================================== */
...
(function () {
  ...
  const C = {
    bg:'#e8eaef', surface:'#f0f2f6', surface2:'#e8eaef',
    border:'#c8ced8', grid:'#d0d5de', txt:'#5a6577',
    heading:'#0f172a', green:'#10b981', red:'#ef4444',
  ...
  async function loadData() {
    try {
      ...
      btnStart.textContent = '✕ Veri bulunamadi';
    }
  }
  ...
  function drawFeatureChart(ch, vs, ve) {
    ...
    for (let i = vs + 1; i < ve; i++) {
      ...
      if (isSpoof !== prevSpoof) {
        ...
        ctx.beginPath();
        ctx.strokeStyle = isSpoof ? C.red : ch.color;
  ...
  function drawModelChart(vs, ve) {
    ...
    for (let i = vs; i < ve - 1; i++) {
      ...
      const y1 = toY(p1), y2 = toY(p2);
      ctx.fillStyle = (p1 + p2) / 2 >= THRESHOLD ? 'rgba(239,68,68,0.12)' : 'rgba(16,185,129,0.05)';
      ctx.beginPath();
      ctx.moveTo(x1, thY); ctx.lineTo(x1, y1); ctx.lineTo(x2, y2); ctx.lineTo(x2, thY);
  ...
  function addMiniLog(idx) {
    ...
    const time = rowToTime(idx, DATA.totalRows);
  ...
  function renderMiniLogs() {
    ...
    for (const e of shown) {
      ...
      html += `<div class="mini-log-entry ${cls}">
        <span class="mini-log-time">${e.time}</span>
        <span class="mini-log-type">${typeLabel}</span>
        <span class="mini-log-prob">${e.prob.toFixed(3)}</span>
      </div>`;
    }
    feed.innerHTML = html;
  }

  /* ── Sync to localStorage (for logs page) ───────────────────────── */
  function syncState() {
    try {
      localStorage.setItem('bkzs_sim', JSON.stringify({
        cursor, tp, fp, fn, tn, running, paused, speed,
        timestamp: Date.now()
      }));
    } catch(e) {}
  }
  ...
  function updateDashboard() {
    ...
    if (running) {
      ...
      if (paused) {
        ...
        txt.textContent = cursor >= total ? 'Tamamlandi' : 'Calisiyor';
      }
    }

    const statusEl = $('sim-status-text');
    if (statusEl) {
      if (cursor >= total) statusEl.textContent = '✓ Analiz tamamlandi';
      else {
  ...
  function startSimulation() {
    ...
    // Logo fly-up animation then reveal sim
  ...
  function resetSimulation() {
    if (!DATA) return;
    cursor = 0; tp = fp = fn = tn = 0;
    paused = false; miniLogs = []; prevLabel = -1;
  ...
  btnSpeedUp.addEventListener('click', () => { speed = Math.min(speed + 5, 100); speedDisp.textContent = speed + 'x'; });
  btnSpeedDown.addEventListener('click', () => { speed = Math.max(speed - 5, 1); speedDisp.textContent = speed + 'x'; });
  btnReset.addEventListener('click', resetSimulation);

  /* ── Keyboard shortcuts ─────────────────────────────────────────── */
  document.addEventListener('keydown', e => {
    if (e.target.tagName === 'INPUT' || e.target.tagName === 'SELECT') return;
    if (e.code === 'Space') { e.preventDefault(); if (running) { paused = !paused; btnPause.textContent = paused ? '▶ Devam' : '⏸ Duraklat'; } }
